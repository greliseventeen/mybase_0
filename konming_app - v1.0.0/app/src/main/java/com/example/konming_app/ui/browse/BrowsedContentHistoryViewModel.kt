...
class BrowsedContentHistoryViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _items = MutableLiveData<List<BrowseHistoryItem.ContentHistory>>()
    val items: LiveData<List<BrowseHistoryItem.ContentHistory>> = _items
    ...
    fun loadData(refresh: Boolean = false) {
        ...
        viewModelScope.launch {
            ...
            val list = withContext(Dispatchers.IO) {
                repo.getContentHistory(userId, currentPage, pageSize)
            }
            if (refresh) {
                _items.value = list
            } else {
